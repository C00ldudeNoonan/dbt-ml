...
class DuckDBAdapter(WarehouseAdapter):
    ...
    filename's stem; if the schema and the catalog collide (both `docbt`)
    ...
    def _ensure_state_table(self) -> None:
        self.connection.execute(
            f"""
            CREATE TABLE IF NOT EXISTS {self.schema_ref}.docbt_state (
    ...
    def materialize_full(self, table: str, df: pl.DataFrame) -> int:
        ...
        self.connection.register("docbt_staging", df)
        try:
            self.connection.execute(
                f"CREATE OR REPLACE TABLE {full} AS SELECT * FROM docbt_staging"
            )
        finally:
            self.connection.unregister("docbt_staging")
        return df.height
    ...
    def materialize_incremental(
        ...
        try:
            ...
            self.connection.execute(f"INSERT INTO {full} SELECT * FROM docbt_staging")
        finally:
            self.connection.unregister("docbt_staging")
        return df.height
    ...
    def list_tables(self) -> list[str]:
        rows = self.connection.execute(
            ...
            "WHERE table_catalog = ? AND table_schema = ? AND table_name != 'docbt_state' "
            "ORDER BY table_name",
