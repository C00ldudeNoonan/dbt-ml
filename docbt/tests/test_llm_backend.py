...
from docbt.backends.llm_backend import LLMBackend
...
def test_llm_pipeline_end_to_end(
    ...
    from docbt.synth import generate_invoice_texts
    ...
    db = project / "target" / "docbt.duckdb"
    con = duckdb.connect(str(db), read_only=True)
    try:
        rows = con.execute(
            'SELECT vendor, total FROM "docbt"."llm_invoices".raw_invoices_llm'
        ).fetchall()
    finally:
        con.close()
    assert len(rows) == 3
