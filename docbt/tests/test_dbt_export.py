...
from docbt.dbt_export import build_dbt_sources, write_dbt_sources
...
def test_basic_shape(fresh_project: Path) -> None:
    ...
    src = payload["sources"][0]
    assert src["name"] == "docbt_invoice_pipeline"
    assert src["database"] == "docbt"  # from duckdb path stem
    assert src["schema"] == "docbt"
...
def test_custom_source_name(fresh_project: Path) -> None:
    payload = build_dbt_sources(fresh_project, source_name="my_docbt")
    assert payload["sources"][0]["name"] == "my_docbt"
...
def test_write_creates_yaml(fresh_project: Path) -> None:
    ...
    assert parsed["sources"][0]["name"] == "docbt_invoice_pipeline"
