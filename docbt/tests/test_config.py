...
from docbt.config import ConfigError, load_project
...
def test_load_example_project(example_project_dir: Path) -> None:
    ...
    assert project.duckdb.schema_name == "docbt"
...
def test_missing_project_file(tmp_path: Path) -> None:
    with pytest.raises(ConfigError, match=r"docbt_project\.yml"):
        load_project(tmp_path)


def test_invalid_yaml_reports_path(tmp_path: Path) -> None:
    (tmp_path / "docbt_project.yml").write_text("name: x\n")
