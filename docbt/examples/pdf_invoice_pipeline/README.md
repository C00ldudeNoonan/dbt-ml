# pdf_invoice_pipeline — the docbt thesis demo
...
The `raw_pdf_text` model uses docbt's `pdf` backend (pypdf, deterministic, no
...
uv run docbt --project-dir examples/pdf_invoice_pipeline seed --count 5
uv run docbt --project-dir examples/pdf_invoice_pipeline run
uv run docbt --project-dir examples/pdf_invoice_pipeline test
uv run docbt --project-dir examples/pdf_invoice_pipeline show extracted_invoices
```
